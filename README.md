...
`upload_github.sh` publishes the **software release**, not your investigations.
It helps create a reproducible portfolio repository with documentation, tests,
licences, dependency wheels, fictional examples and an optional downloadable release.
No GitHub account, repository or credentials are embedded in the script.

### What it does

1. Checks arguments and required programs.
2. Verifies the shipped `PUBLISH_MANIFEST.sha256` against each included file.
3. Copies only matching listed files into a NEW staging directory.
4. Initialises a new Git repository on `main` and displays the staged file summary.
5. Stops here by default: preparation needs no GitHub login or network access.
6. With `--publish`, uses the authenticated GitHub identity, creates the chosen
   NEW repository, and pushes `main` plus tag `v1.4.0`.
7. With `--publish --release`, publishes release notes and attaches a ZIP and checksum.

The staging copy has its own Git history. The script does not run `git add .` in
your original working directory, reuse its history, overwrite existing remote
repositories, force-push, delete branches, or modify your global Git configuration.
Unlisted files such as local databases, `.env` files and newly created reports are
not copied. Only shipped fictional messages are included. The manifest checks an
unchanged release snapshot; it is not a digital signature or a universal secret scanner.

### Step 1 — extract this GitHub-ready package

```bash
unzip MailIntel-v1.4.0-GitHub-ready.zip
cd mailintel-v1.4
```

Keep `upload_github.sh` with `PUBLISH_MANIFEST.sha256` and the complete project.
The standalone script is provided for reading, but cannot publish without those files.
Use this package in a new directory instead of overwriting your working investigation.

### Step 2 — install publishing prerequisites

On Kali/Parrot/Ubuntu, using repositories that provide these packages:

```bash
sudo apt-get update
sudo apt-get install git gh python3 unzip
```

`git init -b main` needs Git 2.28 or newer. If your distribution does not provide
`gh`, follow [GitHub CLI installation instructions](https://github.com/cli/cli#installation).
No Docker, root Git session or GitHub password in a script is needed.

### Step 3 — prepare locally without uploading

Replace `YOUR_USERNAME` with the owner account or organisation that should own the repo:

```bash
bash upload_github.sh --repo YOUR_USERNAME/mailintel
```

The script prints a new staging directory. Inspect its README and staged file list.
This command does not contact GitHub. To choose a persistent preparation path:

```bash
mkdir -p "$HOME/projects"
bash upload_github.sh --repo YOUR_USERNAME/mailintel \
  --output "$HOME/projects/mailintel-preview"
```

The output directory must not already exist. The script does not delete previews.
...
Follow the browser/device-login flow. Do not paste a token into the README or script.
An organisation destination requires permission to create repositories there.

### Step 5 — upload a private repository and release

```bash
OWNER=$(gh api user --jq .login)
bash upload_github.sh --repo "$OWNER/mailintel" \
  --publish --release \
  --output "$HOME/projects/mailintel-published"
```

Run `mkdir -p "$HOME/projects"` first if you skipped Step 3. The chosen repository
name must not exist. The script creates it as **private by default**. Passing
`--publish` is the instruction to perform the upload; there is no additional prompt.
Passing `--release` publishes the release immediately, not as a draft.

For a public portfolio repository, explicitly add the public visibility flag:

```bash
bash upload_github.sh --repo "$OWNER/mailintel" \
  --visibility public --publish --release
```

Choose private OR public creation for the new name, not both commands in succession.
To upload source and the version tag without creating a release, omit `--release`.

### Script options

| Option | Meaning |
|---|---|
| `--repo OWNER/NAME` | Required destination on github.com; new repo only |
| `--visibility private` | Default; explicit private creation |
| `--visibility public` | Anyone can access the repository and release |
| `--publish` | Creates the remote repository and pushes source and tag |
| `--release` | Also publishes ZIP/checksum assets; requires `--publish` |
| `--output PATH` | New staging path; default is a temporary directory |
| `--help` | Shows usage without preparing or uploading |

### After upload

The script prints your repository URL and local Git checkout path. With `--release`,
GitHub's Releases page will contain `MailIntel-v1.4.0-linux.zip` and its SHA256 file.
The README renders on the repository home page. The installable dependency wheels
remain included so a downloaded release can install its core without a package index.
The application version stays 1.4.0; this package adds documentation/publishing files.

The uploader is a **first-publication script for this exact snapshot**. For future
changes, work in a normal clone, review diffs, run tests, update packaging metadata,
commit changes and use a new release tag. Do not force-replace `v1.4.0` or bypass a
checksum mismatch to publish unrelated local files.

### Failure recovery

- **Manifest mismatch:** an included file changed. Start from an untouched copy of
  this GitHub-ready ZIP. Unlisted investigation files are ignored, not a mismatch.
- **Repository already exists:** creation stops; choose another new name or use
  your existing repository's normal Git workflow. There is no overwrite option.
- **Authentication/permission error:** run `gh auth status`; check owner/org access.
- **Repository created but push failed:** the printed staging directory is retained.
  Fix authentication, then run `git -C /path/to/staging push -u origin main` and
  `git -C /path/to/staging push origin refs/tags/v1.4.0`. Do not rerun repo creation.
- **Release creation failed after push:** your source and tag may already be on
  GitHub. Check `gh release view v1.4.0 --repo OWNER/NAME` before creating/uploading
  release assets manually. No automatic rollback or repository deletion occurs.
- **Staging directory already exists:** use a different `--output` or omit it.

The uploader was checked locally. No live GitHub repository was created during
preparation of this deliverable. Application test results are in VALIDATION.md;
publishing-script verification is in PUBLISH_VALIDATION.md.

Official CLI references used for the script:
